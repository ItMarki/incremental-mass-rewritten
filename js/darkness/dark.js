    ...
    */
    ...
    if (tmp.tab == 7) {
        if (tmp.stab[7] == 0) {
            ...
            if (eff.sn) e += `<br>額外給予 <b>x${eff.sn.format(3)}</b> 個超新星`+eff.sn.softcapHTML(7.5,hasElement(9,1))
            if (eff.en) e += `<br>將所得熵提升 <b>x${eff.en.format(3)}</b>`
            if (eff.ab) e += `<br>將深淵之漬獲得量提升 <b>x${eff.ab.format(3)}</b>`
            if (eff.bhp) e += `<br>將黑洞加成公式的指數增加 <b>+${eff.bhp.format(3)}</b><br>移除黑洞指數加成效果的上限`.corrupt(c16)
            ...
            if (tmp.chal14comp) {
                ...
                if (eff.mexp) e += `<br>`+`將所有有色物質獲得量提升 <b>^${eff.mexp.format(3)}</b>`.corrupt(c16)
                if (eff.accelPow) e += `<br>將加速器力量提升 <b>x${eff.accelPow.format(3)}</b>`+eff.accelPow.softcapHTML(5,hasElement(234))
                if (eff.ApQ_Overflow) e += `<br>原子力量和夸克的溢出推遲 <b>^${eff.ApQ_Overflow.format(3)}</b>`.corrupt(c16)
                if (eff.fss) e += `<br>天樞碎片強 <b>${formatPercent(eff.fss-1)}</b>`
                if (eff.ea) e += `<br>將奇異原子的公式提升 ^<b>${format(eff.ea)}</b>`+eff.ea.softcapHTML(1.75)
