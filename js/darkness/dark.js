...
function updateDarkHTML() {
	...
	if (unl) tmp.el.darkAmt.setHTML(player.dark.rays.format(0)+"<br>（"+(og?"+"+dtmp.gain.format(0):"需要 Og-118")+"）")
    ...
    if (tmp.tab == 7) {
        ...
        if (eff.sn) e += `<br>額外給予 <b>x${eff.sn.format(3)}</b> 個超新星`
        ...
        if (tmp.chal14comp) {
            ...
            e = getNextDarkEffectFromID(2) + `
                ...
                <br>將質量軟上限^4-6 推遲 <b>^${eff.msoftcap.format(3)}</b>
            `
