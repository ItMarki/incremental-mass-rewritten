...
const FORMS = {
    ...
    bh: {
        ...
        reset() {
                if (tmp.bh.dm_can) {
                if (player.confirms.bh) createConfirm("你確定要重置嗎？",'bhReset',CONFIRMS_FUNCTION.bh)
...
function formatGain(amt, gain, isMass=false) {
    ...
    if (((ooms.gte(10) && amt.gte('ee100')) || ooms.gte(10**0.05) && amt.gte('ee1000')) && (!isMass || md == 1 || md == 2)) {
		...
		if ((md == 0 || md == 3) && isMass){
			...
			if (getMltValue(gain).log10().div(15).gte(1000) || arv_next.sub(arv_amt).gte(10))rate = "（+"+format(arv_next.sub(arv_amt).mul(20)) + " arv/秒）"
		}
    }
    else rate = "（+"+f(gain)+"/秒）"
	}
...
function overflowFormat(x,inv=false) { return "經過 <b>"+format(x)+"</b> "+(inv?"次方":"次方根")+"的運算" }
