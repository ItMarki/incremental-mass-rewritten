...
const BUILDINGS_DATA = {
    mass_1: {
        ...
        get_effect: x => "質量獲得量 +"+formatMass(x.effect),
    },
    mass_2: {
        name: "提升器",
		icon: "mass_upg2",
        ...
        get_effect: x => "提重器力量 x"+format(x.effect),
    },
    mass_3: {
        name: "增強器",
		icon: "mass_upg3",
        ...
        get_effect: x => "提升器力量 ^"+format(x.effect)+(x.effect.gte(x.ss)?`<span class='soft'>（軟上限${x.effect.gte(1.8e5)?x.effect.gte(5e15)&&!player.ranks.pent.gte(15)?"^3":"^2":""}）</span>`:""),
    },
    mass_4: {
        name: "過強器",
		icon: "mass_upg4",
        ...
        get_power: x => "+^"+format(x.power),
        get_effect: x => "增強器力量 ^"+format(x.effect)+x.effect.softcapHTML(x.ss),
    },
    tickspeed: {
        name: "時間速度",
		icon: "tickspeed",
		...
		+(x.power.gte(x.ss)&&!hasUpgrade('rp',16)?"<span class='soft'>（軟上限）</span>":""),
        get_effect: x => "質量獲得量 "+(hasElement(199) && !CHALS.inChal(15) ? "^"+format(x.effect) : formatMult(x.effect)),
    },
    accelerator: {
        name: "加速器",
		icon: "accelerator",
        ...
        get_power: x => "+^"+format(x.power),
        get_effect: x => "時間速度效果 ^"+format(x.effect)+x.effect.softcapHTML(x.ss),
    },
    bhc: {
        name: "黑洞壓縮器",
		icon: "bhCondenser",
        ...
        get_effect: x => "黑洞質量 "+formatMult(x.effect),
    },
    fvm: {
        name: "假真空操控器（FVM）",
		icon: "fvm",
        ...
        get_power: x => formatMult(x.power),
        get_effect: x => "不穩定黑洞生產速度 "+formatMult(x.effect),
    },
    cosmic_ray: {
        name: "宇宙射線",
		icon: "gamma_ray",
    ...
    star_booster: {
        ...
        get_effect: x => "恆星生產器 "+formatMult(x.effect) + x.effect.softcapHTML('e3e18',hasUpgrade('atom',22)),
    },
    cosmic_string: {
        name: "宇宙弦",
		icon: "cosmic_string",
        ...
        get_power: x => formatMult(x.power),
        get_effect: x => "藍圖粒子 "+formatMult(x.effect),
    },
    pe: {
        name: "平行擠壓器",
		icon: "pe",
        ...
        get_power: x => formatMult(x.power),
        get_effect: x => "維度質量 "+formatMult(x.effect),
    },
...
const BUILDINGS = {
	...
	setup() {
		for (var [i, b] of Object.entries(BUILDINGS_DATA)) {
			...
			if (el.el) el.setHTML(`<div class="table_center upgrade" style="width: 100%; margin-bottom: 5px;">
                ...
                <button class="btn" onclick="BUILDINGS.buy('${i}', true)" style="width: 120px">購買最大量</button>
				...
				<div style="margin-left: 5px; text-align: left; width: 400px">
					力量：<span id="building_pow_${i}"></span><br>
	...
	update(i) {
        ...
        tmp.el["building_cost_"+i].setHTML(allow ? "價格："+b.get_cost(cost) : (b.denyPurchaseText??"") + "鎖定"??"")

        tmp.el["building_auto_"+i].setDisplay(b.autoUnlocked)
        tmp.el["building_auto_"+i].setHTML(player.build[i].auto ? "開" : "關")
