...
function updateCSHTML() {
    ...
    h += `
    定理力量提升 <b>${formatMult(eff.power_mult)}</b>
    <br>定理的第 1-4 種效果的運氣提升 <b>${formatMult(eff.theorem_luck)}</b>
    <br>無限點數生產速度提升 <b>${formatMult(eff.inf_speed)}</b>
    `
    ...
    if (eff.prim_reduce) h += `<br>原始素定理的要求減少 <b>^${format(eff.prim_reduce)}</b>`
