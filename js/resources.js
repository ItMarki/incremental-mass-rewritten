const RESOURCES_DIS = {
    ...
    md: {
        ...
        desc: (gs)=>format(player.md.particles,0)+"<br>"+(player.md.active?"（+"+format(tmp.md.rp_gain,0)+"）":(hasTree("qol3")?formatGain(player.md.particles,tmp.md.passive_rp_gain.mul(gs)):"（未啟動）")),
    ...
    br: {
        ...
        desc: (gs)=>player.qu.rip.amt.format(0)+"<br>"+(player.qu.rip.active||hasElement(147)?hasUpgrade('br',8)?player.qu.rip.amt.formatGain(tmp.rip.gain.div(10).mul(gs)):`（+${tmp.rip.gain.format(0)}）`:"（未啟動）"),
    ...
    dark: {
        ...
        desc: (gs)=>player.dark.rays.format(0)+"<br>"+(hasElement(118)?tmp.dark.rayEff.passive?player.dark.rays.formatGain(tmp.dark.gain.mul(tmp.dark.rayEff.passive).mul(gs)):"（+"+tmp.dark.gain.format(0)+"）":"（需要鿫（⿹气奧）-118）"),
    ...
    inf: {
        ...
        desc: (gs)=>player.inf.points.format(0)+"<br>（+"+tmp.IP_gain.format(0)+"）"+"<br>（"+(tmp.brokenInf?"到下一個無限":"到無限")+"："+formatPercent(player.mass.max(1).log10().max(1).log10().div(tmp.inf_limit.max(1).log10().max(10).log10()).max(0).min(1))+"）",
...
function setupResourcesHTML() {
    ...
    for (i in RESOURCES_DIS) {
        ...
        <div id="${i}_res_hide_div">
            <div><img src="images/${rd.icon||"mass"}.png"><button style="margin-left: 10px; width: 100px;" onclick="hide_res('${i}')" id="${i}_res_hide_btn" class="btn">關閉</button></div>
        </div>
        `
    }
...
function updateResourcesHiderHTML() {
    for (i in RESOURCES_DIS) {
        ...
        if (unl) {
            tmp.el[i+"_res_hide_btn"].setTxt(player.options.res_hide[i] ? "顯示" : "隱藏")
        }
    }
}