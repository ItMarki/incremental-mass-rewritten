...
String.prototype.corrupt = function (active=true) { return active ? this.strike() + ` <span class='corrupted_text'>[已腐化]</span>` : this }
...
function save(){
    ...
    if (tmp.saving < 1) {addNotify("已儲存遊戲", 3); tmp.saving++}
}
...
function exporty() {
    ...
    a.download = "增量質量重寫版存檔 - "+new Date().toGMTString()+".txt"
    a.click()
}
...
function importy() {
    createPrompt("請在此粘貼你的存檔。警告：這樣做會覆蓋你目前的存檔",'import',loadgame=>{
        let st = ""
...
function loadGame(start=true, gotNaN=false) {
    ...
    if (start) {
        ...
        if (tmp.april) createConfirm("你想要移除所有軟上限嗎？",'april',()=>{
            createPopup(`你上當了！我取消不了軟限制！愚人節快樂！<br><br> <img src="https://media.tenor.com/GryShD35-psAAAAM/troll-face-creepy-smile.gif">`,'troll','你幹嘛！')
            ...
            createPopup(`<img style="width: 200px; height: 200px" src="https://media.tenor.com/U1dgzSAQk8wAAAAd/kys.gif">`,'kys','死')
        })
    }
}
...
function checkNaN() {
    ...
    if (naned) {
        ...
        addNotify("由於 "+naned.bold()+"，遊戲資料發生 NaN 錯誤")
...
function simulateTime(sec) {
    ...
    let h = `你離線了 <b>${formatTime(sec)}</b>。<br>`
    ...
    if (s2.mass.gte(10)) h += `<br>你的質量指數<sup>2</sup> 增加了 <b>${s2.mass.format(2)}</b>。`
    else if (s.mass.gte(10)) h += `<br>你的質量指數增加了 <b>${s.mass.format(2)}</b>。`

    if (s2.bh_mass.gte(10)) h += `<br>你的黑洞質量指數<sup>2</sup> 增加了 <b>${s2.bh_mass.format(2)}</b>。`
    else if (s.bh_mass.gte(10)) h += `<br>你的黑洞質量指數增加了 <b>${s.bh_mass.format(2)}</b>。`

    if (s2.quarks.gte(10)) h += `<br>你的夸克指數<sup>2</sup> 增加了 <b>${s2.quarks.format(2)}</b>。`
    else if (s.quarks.gte(10)) h += `<br>你的夸克指數增加了 <b>${s.quarks.format(2)}</b>。`
